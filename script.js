...
const vehicles = Object.keys(data).map(x => x.split('|')[0]).filter((x, i, arr) => i == arr.indexOf(x));
const engines = Object.keys(data).map(x => x.split('|')[1]).filter((x, i, arr) => i == arr.indexOf(x));
...
const addFormElementToPage = () => {
    ...
    let emissions = data[`${document.getElementById(`mode-${id}`).value}|${document.getElementById(`engine-${id}`).value}`];
...
`;
    ...
    let engineSelect = document.getElementById(`engine-${id}`);
       engines.forEach(x => {
        let option = document.createElement('option');
        option.append(document.createTextNode(x));
        option.value = x;
